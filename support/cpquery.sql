Select Top 50
	...
	(Select SCBoards.CutplanID As CutplanIDD, IsNull(LD.LogCount,Cutplans.PlannedLogsForCutplan) As LogCount, IsNull(LD.BoardVol, 0) As BoardVol, SCBoards.* From
		(Select LData.CutplanID As CutplanID, COUNT(LData.BoardVolume) As LogCount, SUM(LData.BoardVolume) As BoardVol FROM 
			(Select CutplanID, BoardVolume From LogData Where DateTime < @datenow) As LData Group by LData.CutplanID)
				As LD Full Outer Join SCBoards On LD.CutplanID = SCBoards.CutplanID Inner Join Cutplans On SCBoards.CutplanID = Cutplans.ID Where Cutplans.DateCreated < @datenow) As
	
	CPSched Inner Join FCBoards On CPSched.CutPlanID = FCBoards.CutPlanID Order By CPSched.CutplanID desc